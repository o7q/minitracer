...
int main(void)
{
    ...
    mt_renderer_set_samples(renderer, 1000);
    ...
    MT_Mesh *light = mt_mesh_create_plane((MT_Vec3){0, -4.9, 0}, (MT_Vec3){0, 0, 0}, (MT_Vec3){1, 1, 1}, mat_light);
    ...
    MT_Mesh *cube = mt_mesh_create_cube((MT_Vec3){1, -1.05, 1}, (MT_Vec3){0, -0.1, 0}, (MT_Vec3){2, 2, 2}, mat_glass);
    mt_world_add_object(world, cube, MT_OBJECT_MESH);
    // MT_Mesh *cube2 = mt_mesh_create_cube((MT_Vec3){1, -1.0, -3}, (MT_Vec3){0, 1, 0}, (MT_Vec3){2, 2, 2}, mat_diffuse_red);
    ...
    while (!WindowShouldClose())
    {
        ...
        for (int y = 0; y < mt_renderer_get_height(renderer); ++y)
        {
            ...
            {
                MT_Vec3 pixel_color = mt_renderer_get_pixel(renderer, x, y, 1);
                color[y * (int)mt_renderer_get_width(renderer) + x] = (Color){pixel_color.x,
                                                                              pixel_color.y,
                                                                              pixel_color.z,
                                                                              255.0f};
            }
